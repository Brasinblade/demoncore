  ...
  if (val > 400 && val < 620) { 
    if (abs(val - lastStabilityVal) < 2) { 
      if (currentMillis - stabilityTimer > 4500) {
        ...
        // --- THE FIX: SILENT HEALING ---
        // Only reset the sleep timer if the correction is major (>10 units)
        // This stops the 4.5s heal cycle from blocking the 25s sleep timer.
        ...
        center = val;            
        persistent_center = center; 
        stabilityTimer = currentMillis; // Reset the 4.5s fuse to prevent immediate re-trigger
      }
    } else {
